...
import { useCreateDAO, useHandleTransactionSuccess } from "@dex/hooks";
...
export function CreateADAOPage() {
  ...
  function GovernanceForm(): string {
    if (type === DAOType.GovernanceToken) return `/${DAORoutes.Create}/${DAORoutes.GovernanceToken}`;
    if (type === DAOType.MultiSig) return `/${DAORoutes.Create}/${DAORoutes.Multisig}`;
    if (type === DAOType.NFT) return `/${DAORoutes.Create}/${DAORoutes.NFT}`;
    return ``;
  }

  function VotingForm(): string {
    if (type === DAOType.GovernanceToken) return `/${DAORoutes.Create}/${DAORoutes.GovernanceTokenVoting}`;
    if (type === DAOType.MultiSig) return `/${DAORoutes.Create}/${DAORoutes.MultisigVoting}`;
    if (type === DAOType.NFT) return `/${DAORoutes.Create}/${DAORoutes.NFTVoting}`;
    return ``;
  }

  function ReviewForm(): string {
    if (type === DAOType.GovernanceToken) return `/${DAORoutes.Create}/${DAORoutes.GovernanceTokenReview}`;
    if (type === DAOType.MultiSig) return `/${DAORoutes.Create}/${DAORoutes.MultisigReview}`;
    if (type === DAOType.NFT) return `/${DAORoutes.Create}/${DAORoutes.NFTReview}`;
    return ``;
  }
  ...
  const steps = [
    {
      label: "Details",
      route: `/${DAORoutes.Create}/${DAORoutes.DAODetails}`,
      validate: async () => trigger(["name", "logoUrl", "isPublic", "description"]),
    },
    {
      label: "Type",
      route: `/${DAORoutes.Create}/${DAORoutes.Type}`,
