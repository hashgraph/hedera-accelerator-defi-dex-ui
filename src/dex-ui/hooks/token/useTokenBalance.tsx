...
type UseTokenBalanceQueryKey = [TokenQueries.TokenBalance, "balance"];
...
export function useTokenBalance(props: UseTokenBalanceProps) {
  ...
  const provider = DexService.getProvider(context.network, wallet.topicID, walletId ?? "");

  return useQuery<AccountBalanceJson | undefined, Error, number, UseTokenBalanceQueryKey>(
    [TokenQueries.TokenBalance, "balance"],
    async () => {
